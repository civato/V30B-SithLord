...
static void hook_det_work(struct work_struct *work)
{
    ...
    if(is_hook_test == false){
        hook_adc_value = 20;//20; sahoon.kim value change reduce 250->20 : hook pressed when goto sleep.
    }
    else{
...
static irqreturn_t headset_hook_int_handler(int irq, void *dev_id)
{
    ...
    printk(KERN_ERR "[Soc-jack.c][headset_hook_int_handler()]##  headset type %d, detecting %d, block_hook_int %d gpio_hook %d \n", 
			headset_type, headset_detecting, block_hook_int, gpio_hook);
...
//                                             
    ...
    if( (headset_status == HEADSET_CONNECTED) && (headset_type == STAR_NONE))
    {
        ...
        printk(KERN_INFO "##[Soc-jack][gpio_work()]## : hookkey status(%d) gpio %d \n", hookkey_gpio_status, headset_sw_data->gpio);
